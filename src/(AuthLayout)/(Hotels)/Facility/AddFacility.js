...
const AddFacility = ({getFacilities, handleCancel}) => {
    ...
    `;
        ...
        try {
          ...
          console.log(res);
          if (res) {
              message.success("Facility has been Added Successfully")
              getFacilities()
              handleCancel()
              setFormData({})
          }
  ...
  return (
    <form className="w-full h-full p-4">
        <h1 className="title capitalize">add facility</h1>
      <label className="labelStyle mt-4">Facility</label>
      <Input name="name" value={name} onChange={onChange} placeholder="type facility name here" className="w-full border-black"/>
