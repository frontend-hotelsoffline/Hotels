...
const RelAndAllotment = ({id, categoryData, handleCancel, getAllContractData}) => {
    ...
    `;
        ...
        try {
          ...
          if (res && !res.errors) {
              ...
              handleCancel()
              setFormData({})
          }else{message.error(res?.errors?.message)}
        } catch (error) {
  ...
  return (
    <form className="w-full h-full p-4">
      ...
      <Input name="allotment" value={allotment} onChange={onChange} className="w-full border-black"/>
      <label className="labelStyle mt-4">Rel</label>
      <Input name="rel" value={rel} onChange={onChange} className="w-full border-black"/>
